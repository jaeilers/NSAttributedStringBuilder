...
public protocol AttributedStringBuilding {
    ...
    /// Adds new attributes to the current attributed string. The new attributes are applied to the whole range and existing attributes are overridden.
    /// - Parameters:
    ...
    /// Adds an attributed string with its attributes to the current string. The string is appended and the attributes of both strings are merged.
    ...
    /// - Parameters:
    ///   - trait: The new font trait that will be added to the existing font. Default font is `.preferredFont(forTextStyle: .body)`.
